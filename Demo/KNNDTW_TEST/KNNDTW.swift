...
public class KNNDTW: NSObject {
    ...
    private func dtw_cost(y: [Float], x: [Float]) -> Float {
        ...
        for (i,_) in y.enumerate() {
            for (j,_) in x.enumerate() {
    ...
    public func predict(curve_to_test: [Float]) -> knn_certainty_label_pair {
        ...
        */
        ...
        for pair in self.curve_label_pairs {
            distances.append(knn_distance_label_pair(distance: self.dtw_cost(pair.curve, x: curve_to_test), label: pair.label))
        }
        
        //sort the distances, ascending distances
        distances = distances.sort({ (a, b) -> Bool in
        ...
        */
        ...
        let sorted_votes = votes.maxElement({ (a, b) -> Bool in
            if (a.1 < b.1) {
                return true
            }
            else {
